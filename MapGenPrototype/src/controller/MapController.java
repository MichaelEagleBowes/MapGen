...
public class MapController extends Controller {
	...
	/**
	 * Index of the tab that is currently opened.
	 */
	private int loadedTabIndex = 0;
	 ...
	 * <br>
	 * Image Width & Height to be rendered have to be selected.<br>
	 * <br>
	 * Soliciting map sizes are 2^x+1;<br>
	 * e.g. 5, 9, 17, 33, 65, 129, 257, 513, 1025,
	 * 2049, 4097, 8193.
	 *
	 */
	...
	/**
	 * 
	 * Generates the ground of a map with the Cellular Automaton algorithm.<br>
	 * <br>
	 * Image Width & Height to be rendered have to be selected.<br>
	 * All map sizes are valid.
	 *
	 */
	...
	public Image drawCave(int[][] map, int width, int height) {
		...
		Image image = Util.convertBufferedImage(bi);
	 ...
	 * created, but the edges will be cut off in the view.
	 * 
		...
		try {
			...
			for (int i = 0; i < map.length; i++) {
				for (int j = 0; j < map.length; j++) {
					if (map[j][i] <= diamondSquare.getMinimum() + diamondSquare.getOceanSpectrum() && oceanParam > 0)
					{
						ig2.drawImage(dw, i * 32, j * 32, null);
					} else if (map[j][i] <= diamondSquare.getMinimum() + diamondSquare.getOceanSpectrum()
							+ diamondSquare.getCoastSpectrum() && coastParam > 0)
					{
						ig2.drawImage(w, i * 32, j * 32, null);
					}

					else if (map[j][i] <= diamondSquare.getMinimum() + diamondSquare.getOceanSpectrum()
							+ diamondSquare.getCoastSpectrum() + diamondSquare.getBeachSpectrum() && beachParam > 0)
					{
						ig2.drawImage(beach, i * 32, j * 32, null);
							...
							+ diamondSquare.getGrassSpectrum() && grassParam > 0)
					{
						ig2.drawImage(green, i * 32, j * 32, null);
							...
							+ diamondSquare.getCoastSpectrum() + diamondSquare.getBeachSpectrum()
							+ diamondSquare.getGrassSpectrum() + diamondSquare.getForestSpectrum() && forestParam > 0)
					{
						ig2.drawImage(darkgreen, i * 32, j * 32, null);
							...
							+ diamondSquare.getMountainSpectrum() && mountainParam > 0)
					{
						ig2.drawImage(mountain, i * 32, j * 32, null);
					} else if (map[j][i] <= diamondSquare.getMinimum() + diamondSquare.getOceanSpectrum()
							+ diamondSquare.getCoastSpectrum() + diamondSquare.getBeachSpectrum()
							+ diamondSquare.getGrassSpectrum() + diamondSquare.getForestSpectrum()
							+ diamondSquare.getMountainSpectrum() + diamondSquare.getSnowSpectrum() && snowParam > 0)
					{
	...
	public void initialize(Stage stage, HostServices hostServices, MainController mainController) {
		...
		tabPane.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
			...
			public void changed(ObservableValue<? extends Number> ov, Number oldValue, Number newValue) {
				if ((int) newValue == 0) {
					loadedTabIndex = 0;
					getMainController().getControlsController().loadTab(0);
				}
				if ((int) newValue == 1) {
					loadedTabIndex = 1;
					getMainController().getControlsController().loadTab(1);
				}
			}
		});
	}
